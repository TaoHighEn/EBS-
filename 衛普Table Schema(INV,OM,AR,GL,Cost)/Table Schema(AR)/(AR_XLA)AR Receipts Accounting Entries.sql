SELECT
   ...
   WHERE 1 = 1
    ...
    -------- 以下3 個欄位條件才能用到 xla.xla_transaction_entities_N1 INDEX 
     ...
     AND acr.cash_receipt_id            =  nvl( xte.source_id_int_1  , -99)
    -----  AND acr.cash_receipt_id            =   xte.source_id_int_1    ----注意與以上這個條件的不同點
--
     AND xte.application_id             =  222                     ---- 222 = AR
--
     AND xte.application_id             =  xe.application_id(+)  
     AND xte.entity_id                  =  xe.entity_id(+)             ---- 一張 Receipt 可能會有二個以上的event_id 
--
